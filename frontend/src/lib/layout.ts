  ...
  const normalizedNodes = nodes.map(({ node, size, x, y }) => {
    ...
    return {
      ...
      data: {
        ...
        tier: value(node.data, 'tier') || undefined,
        tierConfidence: value(node.data, 'tierConfidence') || undefined,
        kind: node.type,
...
function layoutGroup(
  ...
  const root = {
    ...
    data: {
      ...
      detail: description.detail,
      status: description.status,
      tier: value(groupNode.data, 'tier') || undefined,
      tierConfidence: value(groupNode.data, 'tierConfidence') || undefined,
      kind: groupNode.type,
