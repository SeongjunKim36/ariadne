...
| # | 항목 | 상태 | 완료일 | 비고 |
|---|---|---|---|---|
| 2-10 | IAM Role 수집기 (서비스 연결 역할) + HAS_ROLE 관계 | [x] | 2026-04-14 | EC2 instance profile / ECS task role / Lambda execution role 수집 + `iam-role` 노드 시각화 |
...
| 2-15 | 플러그인 활성화 시 추가 IAM 안내 | [x] | 2026-04-15 | preflight `warningMessage`와 scan warning에 nginx SSM 권한 안내 추가, `iam/ariadne-nginx-plugin-policy.json` 제공 |
...
| 5-10 | Portfolio 리포 업데이트 | [x] | 2026-04-17 | `Portfolio` 리포에서 `portfolio-ariadne-showcase` 브랜치로 Ariadne 지원 카드 추가, PR #8 머지 |

**Phase 5 진행률**: 10/10 (100%)
