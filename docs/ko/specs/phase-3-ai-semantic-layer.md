...
- [ ] 규칙 기반 감사 엔진이 SG/IAM 위험 항목을 High/Medium/Low로 보고
- [ ] "결제 관련 인프라 보여줘" 같은 자연어 질의 → 관련 서브그래프 반환
- [ ] LLM이 인프라를 "웹/앱/DB/캐시/배치" 계층으로 자동 레이블링
- [ ] 5대 질문 #3("prod vs staging 차이") + #4("위험한 SG") 에 답 가능
- [ ] LLM 환각이 사용자에게 도달하지 않음 (Cypher 검증 레이어)
